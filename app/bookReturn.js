...
export default function Page() {
    ...
    useEffect(() => {
        fetch(url + '/api/v1/book/'+ id +'/return', {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    return (
        <ImageBackground source={require('../assets/image/back.png')} style={styles.container}>
            <View style={styles.main}>
                ...
                <Text style={styles.subtitle}>Vous venez de rendre {users.title}</Text>
                <Image
                    style={styles.image}
                    source={'https://small-parks-cheat-193-252-172-28.loca.lt/images/cover/'+ users.cover}
                />
                <Pressable style={styles.button}>
                    <Link href={{ pathname: 'return' , params: { id: previousId }}}>Rendre un autre livre</Link>
                </Pressable>
            </View>
        </ImageBackground>
    );
